...
contract Aggregator is AggregatorInterface, ChainlinkClient, Ownable {
    ...
    uint256 private constant MAX_ORACLE_COUNT = 28;
        ...
        onlyOwner()
    {
        ...
        require(
            justMid.transferFrom(address(this), _recipient, _amount),
    ...
    function cancelRequest(
        ...
        uint256 _payment,
        uint256 _expiration
    ) external ensureAuthorizedRequester() {
        uint256 answerId = requestAnswers[_requestId];
        require(
            answerId < latestCompletedAnswer,
            "Cannot modify an in-progress answer"
        );
    ...
    modifier ensureMinResponsesReceived(uint256 _answerId) {
        if (
            answers[_answerId].responses.length >=
            answers[_answerId].minimumResponses
        ) {
            _;
        }
    }
    ...
    modifier ensureAllResponsesReceived(uint256 _answerId) {
        if (
            answers[_answerId].responses.length ==
            answers[_answerId].maxResponses
        ) {
            _;
        }
    }
    ...
    modifier validateAnswerRequirements(
        ...
        require(
            _oracles.length <= MAX_ORACLE_COUNT,
            "cannot have more than 45 oracles"
        );
        require(
            _oracles.length >= _minimumResponses,
            "must have at least as many oracles as responses"
        );
        require(
            _oracles.length == _jobIds.length,
            "must have exactly as many oracles as job IDs"
        );
        _;
    }
